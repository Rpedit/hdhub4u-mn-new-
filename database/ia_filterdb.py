            ...
            if primary_db_size >= 407:
                ...
                if secondary_db_size >= 407:
                    if db3 and Media3:
                        ...
                        if tertiary_db_size >= 407:
                            if db4 and Media4:
                                saveMedia = Media4
                                target_db = "Quaternary"
                                logger.warning("Switching to Quaternary DB due to size threshold.")
                            else:
                                logger.warning("DB4 not available, staying on DB3 despite size threshold.")
                                saveMedia = Media3
                                target_db = "Tertiary"
                        else:
                            saveMedia = Media3
                        ...
                        logger.warning("DB3 not available, staying on DB2 despite size threshold.")
                        saveMedia = Media2
                        target_db = "Secondary"
                else:
                    saveMedia = Media2
...
async def get_search_results(chat_id, query, file_type=None, max_results=None, offset=0, filter=False):
    ...
    if ULTRA_FAST_MODE:
        ...
        if MULTIPLE_DB:
            ...
            if Media3:
                find_tasks.append(Media3.find(filter_mongo).sort("$natural", -1).skip(offset).limit(limit).to_list(length=limit))
            if Media4:
                find_tasks.append(Media4.find(filter_mongo).sort("$natural", -1).skip(offset).limit(limit).to_list(length=limit))
        ...
        # Sort concatenated results by natural order (assuming new files are naturally added at end)
        # Note: Since we fetch sorted("$natural", -1), we have [LatestDB1, LatestDB2...]
        # If DB1 is old and DB2 is new, DB1 Latest are actually older than DB2 Latest.
        # But usually we fill DB1 then DB2. So DB2 has newer files.
        # So results[0] (DB1) has older files than results[1] (DB2).
        # We constructed `files` as DB1_files + DB2_files + ...
        # If we want "Latest" global, we should probably reverse the order of extending or re-sort.
        # However, to preserve "natural" feel where we see results from all DBs, the current appending is tricky.
        # But if the user wants "Most Recent", we should prioritize DB4 > DB3 > DB2 > DB1.
        # The current implementation (since my changes) appends DB2, DB3, DB4.
        # So files = DB1 + DB2 + DB3 + DB4.
        # If we take [:limit], we take DB1.
        # This means we see OLD files first.
        # I will change the logic to prepend or re-order.

        # New Logic: Aggregate all then sort? No, too expensive if many results.
        # Better: Reverse the task order or extend order.
        # Actually, let's just collect all results then take the best ones?
        # Since we use $natural -1, we are getting the "newest" from EACH db.
        # Newest of DB4 is newer than Newest of DB1 (if DB1 is full and we are writing to DB4).
        # So we should order: DB4 results, DB3 results, DB2 results, DB1 results.
        ...
        if MULTIPLE_DB:
            ...
            if Media3:
                count_tasks.append(Media3.count_documents(filter_mongo))
                find_tasks.append(Media3.find(filter_mongo).sort("$natural", -1).skip(offset).limit(max_results).to_list(length=max_results))
            if Media4:
                count_tasks.append(Media4.count_documents(filter_mongo))
                find_tasks.append(Media4.find(filter_mongo).sort("$natural", -1).skip(offset).limit(max_results).to_list(length=max_results))
        ...
        if MULTIPLE_DB and len(find_results) > 1:
            files = []
            for result in reversed(find_results):
                files.extend(result)
        else:
...
async def get_bad_files(query, file_type=None):
    ...
    if MULTIPLE_DB:
        ...
        if Media3:
            ...
            files3 = []
        if Media4:
            cursor4 = Media4.find(filter).sort('$natural', -1)
            files4 = await cursor4.to_list(length=(await Media4.count_documents(filter)))
        else:
            files4 = []
...
async def get_file_details(query):
    ...
    if MULTIPLE_DB:
        ...
        if Media4:
            tasks.append(Media4.find(filter).to_list(length=1))
            ...
            if db_size > 407:
                ...
                if db2_size > 407:
                    if db3 and Media3:
                        ...
                        if db3_size > 407:
                            if db4 and Media4:
                                cursor = Media4.find().sort("$natural", -1).limit(limit)
